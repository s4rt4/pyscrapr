...
1. **Buka Selector Playground** — sidebar "Playground". Ekspektasi: form dengan 3 field utama (URL, Selector Type, Selector).
2. **Tempel URL target** — format lengkap dengan protokol `https://example.com/products`. Ekspektasi: input text tidak memvalidasi URL sebelum submit; invalid URL akan error saat Run.
3. **Pilih Selector Type** — radio/toggle CSS atau XPath. Default CSS. Ekspektasi: placeholder selector berubah sesuai jenis (`div.class` untuk CSS, `//div[@class='...']` untuk XPath).
4. **Tulis selector** — atau klik salah satu Quick Example. Ekspektasi: text field aktif.
5. **Klik Run** — backend fetch + parse. Ekspektasi: loading spinner 1-5 detik tergantung ukuran halaman.
6. **Review match count di atas result list** — misal "3 matches found". Ekspektasi: angka jelas.
7. **Expand setiap result** — lihat text content (cleaned whitespace), atribut (semua key-value), dan HTML preview (truncated 500 karakter). Ekspektasi: collapse/expand smooth.
8. **Refine selector jika tidak sesuai** — misal dapat 0 match, tambah wildcard; dapat terlalu banyak, tambah class prefix.
9. **Klik "Use in Harvester" jika sudah yakin** — redirect ke halaman Image Harvester dengan field URL dan optionally selector sudah terisi. Ekspektasi: URL param `?url=...` terbawa.
10. **Atau copy selector manual** — untuk pemakaian di Custom Pipeline atau AI Extract. Ekspektasi: tidak ada tombol copy otomatis untuk selector, pakai keyboard.
11. **Ganti URL untuk test lain** — Playground tidak stateful, ganti URL lalu Run lagi.
12. **Close tab tanpa save** — tidak ada data yang hilang karena memang tidak disimpan.
