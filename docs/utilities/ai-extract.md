...
PyScrapr menyediakan empat preset template yang siap pakai: `Product info` (extract nama, harga, SKU, deskripsi, stok dari halaman e-commerce), `Article summary` (judul, author, tanggal publish, summary, key points, sentiment dari artikel berita/blog), `Named entities` (ekstraksi: people, organizations, locations, dates, money amounts dari teks bebas), dan `Contact info` (nama, email, telepon, alamat, company, role dari halaman "tentang kami" atau business card). Anda bisa memilih preset sebagai titik awal lalu memodifikasi schema sesuai kebutuhan - preset bukan hardcoded, hanya starter JSON.
...
3. **Buka halaman AI Extract di PyScrapr** - dari sidebar pilih `AI Extract (LLM)`. Ekspektasi: status card di atas langsung melakukan health-check ke Ollama dan menampilkan badge `running` plus daftar model yang tersedia.
4. **Pilih model dari dropdown** `Model` - biasanya `llama3.2:latest` atau model lain yang sudah Anda pull. Ekspektasi: dropdown terisi otomatis dari response `/api/tags` Ollama.
5. **Pilih preset atau tulis schema kustom** - pilih salah satu preset dari dropdown (`Product info`, `Article summary`, `Named entities`, `Contact info`) untuk auto-fill, atau tulis sendiri di field `Schema description` dalam format deskripsi JSON seperti `{"nama": "nama lengkap orang", "umur": "umur dalam tahun (integer)"}`. Ekspektasi: textarea terisi dengan schema starter.
6. **Tempelkan teks input** - paste HTML yang sudah di-clean, artikel, atau teks mentah apapun ke field `Text to analyze`. Batas praktis ~8000 karakter untuk model 3B-7B agar tidak truncate context. Ekspektasi: counter karakter di bawah tombol menunjukkan jumlah.
...
8. **Review hasil di panel `Result`** - hasil muncul sebagai JSON formatted di panel kanan. Ekspektasi: JSON valid, sesuai schema, tanpa field ekstra.
9. **Copy hasil** - seleksi isi JSON di panel `Result` dan salin ke clipboard secara manual. (Tombol `Copy JSON` dedicated belum tersedia di UI saat ini.)
...
### Problem: Status card merah `offline`
- **Symptom**: badge `offline` merah, tombol `Extract` disabled, alert kuning "Ollama not running".
...
- [System requirements](/docs/getting-started.md) - spec hardware minimum untuk model LLM.
