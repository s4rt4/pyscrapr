...
1. **Buka Auth Vault** — sidebar "Vault". Ekspektasi: tabel daftar profile yang sudah ada.
2. **Klik "New Profile"** — modal form muncul. Ekspektasi: field Domain, Cookies (JSON editor), Headers (JSON editor), Notes (textarea).
...
4. **Tambahkan Cookies** — paste JSON array. Minimal field: `name`, `value`. Opsional: `domain`, `path`, `expires`, `secure`, `httpOnly`. Ekspektasi: editor JSON dengan syntax highlighting, validasi saat save.
5. **Tambahkan Headers** — paste JSON object key-value. Contoh: `{"Authorization": "Bearer eyJ...", "User-Agent": "MyCustomUA/1.0"}`. Ekspektasi: format object standard.
6. **Tulis Notes** — kapan cookie ini expire, dari akun mana, dll. Ekspektasi: bebas text.
7. **Save** — upsert ke `auth_vault.json`. Ekspektasi: toast sukses, modal close, tabel refresh dengan entry baru.
8. **Alternatif: Import dari Browser** — klik "Import from Browser", pilih browser (Chrome/Firefox/Edge/Brave), optionally isi domain filter. Ekspektasi: warning "close browser first", lalu progress import.
...
11. **Edit existing profile** — klik row di tabel → modal edit muncul dengan data saat ini. Ubah, save.
12. **Hapus profile** — tombol Delete, konfirmasi `window.confirm` browser. Ekspektasi: entry hilang dari JSON dan tabel.
...
Array JSON. Setiap item minimal:
```json
{
  "name": "session_id",
  "value": "abc123xyz",
...
```
Field `expires` (Unix timestamp) opsional; kalau ada, cookie akan di-skip oleh httpx jika sudah expired. `domain` cookie bisa berbeda dari domain profile (berguna untuk cookie yang di-set oleh subdomain tapi valid untuk parent).
