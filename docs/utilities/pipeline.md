...
1. **Buka halaman Custom Pipeline** — dari sidebar pilih "Pipeline". Ekspektasi: tabel daftar pipeline yang sudah ada (kosong kalau belum pernah buat).
2. **Klik "New Pipeline"** — form baru muncul. Ekspektasi: editor Monaco kosong dengan placeholder komentar.
3. **Isi metadata** — Nama (bebas), Description (opsional tapi sangat disarankan), Auto Run On (None/Image Harvester/URL Mapper/Site Ripper/Media Downloader). Ekspektasi: field tersimpan di state lokal.
4. **Tulis skrip di Monaco Editor** — atau klik salah satu preset snippet: `filter_size`, `regex_clean`, `extract_domain`, `aggregate_stats`. Ekspektasi: editor terisi kode starter dengan komentar penjelasan.
...
6. **Pilih sumber data untuk Test Run** — di panel bawah: (a) supply `job_id` dari job yang sudah selesai, pipeline akan load hasil job itu; atau (b) paste sample JSON manual ke textarea. Ekspektasi: validasi JSON otomatis, error merah jika malformed.
7. **Klik "Run Test"** — skrip dieksekusi di backend. Ekspektasi: loading spinner 0.5-5 detik, lalu panel Result muncul dengan stdout capture (dari `print()`) + hasil `data`/`output` final.
8. **Iterasi** — ubah skrip, Run Test lagi. Tidak perlu save dulu.
9. **Save pipeline** — klik tombol "Save". Ekspektasi: entry baru/update di `data/pipelines.json`, toast "Saved".
10. **Aktifkan Auto Run (opsional)** — kembali ke form, set `auto_run_on` ke job type yang diinginkan, save lagi. Ekspektasi: setelah ini, setiap job type terkait yang completed akan memicu pipeline.
11. **Monitor hasil auto-run** — di panel "Recent Runs", atau cek `data/pipeline_runs/{job_id}_{pipeline_id}.json` di disk.
12. **Disable / hapus jika tidak perlu** — edit dan set `auto_run_on = None`, atau klik tombol Delete di tabel.
...
- [Event bus architecture](../system/events.md) — detail teknis pipeline_listener.
