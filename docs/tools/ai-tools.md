...
2. Di field **Source**, pilih job Image Harvester sebelumnya dari dropdown (sudah completed). Atau upload folder lokal via tombol **Browse Folder**. Tool akan scan dan tampilkan count gambar yang valid.

3. Masukkan **Labels** — satu label per baris di text area. Contoh:
   ```
...
4. Atur **Threshold confidence** (slider 0-1, default 0.25). Label dengan score di bawah threshold tidak ditampilkan di hasil. Naikkan untuk hasil lebih strict, turunkan untuk lebih inclusive.

5. Opsional: toggle **Single-label mode**. Default OFF (multi-label, gambar bisa match beberapa label). Kalau ON, hanya label dengan score tertinggi yang di-assign per gambar.

6. Klik **Start Classification**. Pada run pertama, tool download model CLIP (~350MB) — progress bar muncul. Download ini sekali saja, setelahnya cached.

7. Setelah model loaded, classification mulai. Panel kanan menampilkan progress: `X / Y images processed, ETA Zmin`. Setiap gambar processing ~1 detik di CPU.

8. Selama running, hasil populate incrementally sebagai grid. Setiap card berisi thumbnail + confidence bars per label (colored progress bar dengan percent).

9. Setelah selesai, **Tag filter chips** muncul di bagian atas panel — satu chip per label. Klik chip untuk filter grid hanya tampilkan gambar yang match label tersebut.

10. Klik thumbnail untuk modal detail: full-size preview, ranking semua label dengan exact score, metadata original (URL source, filename, dimensions).

11. Untuk export: tombol **Export CSV** generate tabel `filename,label,confidence` untuk analisis spreadsheet. **Export ZIP by label** membuat folder per-label dan copy gambar matching ke tiap folder.

12. Hasil classification tersimpan di History dengan link ke source Harvester job. Anda bisa re-run dengan labels berbeda tanpa re-process image encoding (cached).
...
- [Pipeline](../advanced/pipeline.md) — auto-organize by label hasil
