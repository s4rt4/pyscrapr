...
2. Paste **URL** video/playlist/channel di text field. Mendukung banyak platform: YouTube, YouTube Music, Instagram (post/reel/story), TikTok, Twitter/X, Facebook, Vimeo, Twitch, Reddit, SoundCloud, Bandcamp, Bilibili, dll. Ekspektasi: auto-detect platform muncul di bawah field.

3. Klik tombol **Probe** (ikon kaca pembesar). Backend jalankan `extract_info` tanpa download. Dalam 2-10 detik muncul card preview: thumbnail, title, channel, duration, upload date, available quality list, subtitle availability.

4. Pilih **Quality preset** dari dropdown: `best` (quality tertinggi available), `4k` (maks 2160p), `1080p`, `720p`, `480p`, atau `audio` (audio-only). Ekspektasi: estimasi file size muncul berdasarkan pilihan.
...
6. Pilih **Subtitles** mode: `skip` (tidak download), `download` (save sebagai .srt terpisah), `embed` (embed ke container MKV). Kalau pilih download/embed, masukkan **Languages** comma-separated: `en,id,ms`.

7. Untuk playlist: set **Playlist range** — `start` (item ke-N mulai), `end` (sampai item ke-M), `max_items` (batasi total). Default: download semua. Ubah untuk hemat bandwidth atau testing.

8. Opsional advanced: toggle **Cookie from browser** dan pilih source (Chrome/Firefox/Edge/Brave). Aktifkan hanya untuk konten behind login.
...
12. Export ke folder tujuan via **Open Folder**, atau tambahkan ke playlist player eksternal via path yang di-copy. History otomatis menyimpan job untuk re-download atau resume kalau interrupted.
...
- [Pipeline: Audio Processing](../advanced/pipeline-audio.md) — convert, normalize, transcribe
- [Image Harvester](image-harvester.md) — untuk thumbnail/gambar artikel
- [AI Tools](ai-tools.md) — klasifikasi thumbnail video
- [Scheduler](../advanced/scheduler.md) — auto-download channel update
