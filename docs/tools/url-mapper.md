...
2. Di field **Seed URL**, masukkan alamat awal crawl. Contoh: `https://example.com`. URL ini jadi root node di tree output. Ekspektasi: ikon checkmark hijau muncul jika URL reachable (preflight HEAD request).
...
6. Set **Rate limit (req/sec per host)**. Default 2 request/detik. Untuk site besar yang bisa handle load, naikkan ke 5-10. Untuk WordPress shared hosting, turunkan ke 1.

7. Opsional: Di tab **Advanced**, set **Exclude URL patterns** (regex list) untuk skip seksi tertentu — misal `/tag/`, `/author/`, `/search?`, `/page/\d+` untuk hindari duplikasi pagination.

8. Klik **Mulai Crawl**. Backend creates job, returns job_id, SSE stream terbuka. Ekspektasi: live counter muncul menampilkan "Discovered: X, Visited: Y, Queued: Z".
...
11. Setelah crawl selesai, pindah ke tab **Graph View** untuk visualisasi Cytoscape.js dengan layout cola. Drag-drop node, zoom, pan. Klik node untuk buka detail drawer dengan metadata: response time, content-type, status code, title, inbound/outbound links.

12. Export hasil via dropdown **Export**: pilih PNG (screenshot graph), JSON (machine-readable semua data), atau XML (Google Sitemap compatible). File tersimpan di folder output.
...
- [Scheduler](../advanced/scheduler.md) — automate weekly crawl untuk monitoring
- [History & Compare](../system/history.md) — diff hasil crawl antar waktu
- [Export formats](../system/export.md) — detail format XML sitemap, JSON, CSV
