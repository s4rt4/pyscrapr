...
2. Masukkan `Target URL`, halaman entry point yang akan dijadikan homepage offline mirror. Contoh: `https://docs.example.com`. Ekspektasi: URL validasi real-time + preview title via HEAD request.

3. Set `Max depth` (slider 0-5). Depth 2 cukup untuk docs kecil. Depth 4 untuk situs medium. Di atas itu, expect mirror multi-GB.

4. Toggle `Stay on domain` (default ON) dan `Include external assets` (default ON). Kalau Anda butuh mirror yang benar-benar self-contained termasuk CDN font/lib, pastikan `Include external assets` aktif.

5. Atur toggle `Rewrite links` dan `Generate PDF report` sesuai kebutuhan. `Rewrite links` mengubah URL jadi relative path, `Generate PDF report` menghasilkan laporan otomatis setelah rip selesai.

6. Set `Max pages` dan `Max assets` sebagai safety cap agar rip tidak meledak di site besar.

7. Set `Concurrency`, jumlah asset paralel. Default 6. Site Ripper lebih berat dari Harvester, jangan naik di atas 12. Atur juga `Rate /s per host` untuk menghindari rate limit.

8. Klik `Start rip`. Backend create job dengan 2 sub-phase. Panel kanan menampilkan phase indicator: **Phase 1/2: Crawling & Downloading**.
...
11. Setelah selesai, ringkasan muncul: total files, total size, broken links count. Buka file `site/index.html` di folder output untuk navigate mirror offline di browser.

12. Download `PDF report` via dropdown `Download` di header, otomatis ter-generate dengan chart dan tabel. Atau pilih `Mirror ZIP` untuk archive portable lengkap.
...
- [Pipeline](/docs/utilities/pipeline.md) - minify, optimize asset post-rip
- [Scheduler](/docs/system/scheduled.md) - automate periodic archival
- [Export & ZIP](/docs/system/history.md) - packaging untuk distribution
