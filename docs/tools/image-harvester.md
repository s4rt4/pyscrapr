...
2. Di field **URL target**, masukkan alamat halaman yang ingin di-harvest. Contoh: `https://unsplash.com/s/photos/mountain`. Tool ini menerima URL tunggal atau multiple URLs (satu per baris). Ekspektasi output: URL tervalidasi secara real-time — kalau format salah, ikon warning merah muncul di kanan field.
...
4. Set **Minimum width** dan **Minimum height** dalam pixel. Default 200x200. Untuk harvesting foto hi-res naikkan ke 800x600 atau lebih. Untuk icon kecil turunkan ke 32x32. Tool akan menolak file yang dimensinya di bawah threshold ini.

5. Set **Minimum bytes** (ukuran file minimum). Default 10240 (10KB). Ini mencegah download placeholder 1KB atau tracking pixel.
...
9. Klik tombol **Mulai Harvest** (warna biru besar di bawah form). Backend akan membuat job entry di database dan mengembalikan `job_id`. Ekspektasi: tombol berubah jadi disabled + spinner, dan panel kanan mulai menampilkan progress bar.
...
12. Klik tombol **Export ZIP** di header untuk mengunduh semua gambar dalam satu archive, atau klik **Open Folder** untuk membuka lokasi file di Windows Explorer. Job ini juga otomatis tercatat di History dan bisa di-replay kapanpun.
...
- [Pipeline & Post-processing](../advanced/pipeline.md) — konversi, resize, strip metadata
- [Scheduler](../advanced/scheduler.md) — jadwalkan harvest periodik
